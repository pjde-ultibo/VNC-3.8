...
procedure THelper.VNCRect (Sender: TObject; r: Classes.TRect);
begin
  Count := Count + 1;
  ConsoleWindowSetXY (Console3, 1, 3);
  Consolewindowwrite (Console3, 'FRAME ' + IntToStr (Count) + '   ');
  aVNC.Canvas.Fill (SetRect (20, 400, 200, 470), COLOR_RED);
  aVNC.Canvas.DrawText (30, 438, 'FRAME ' + Count.ToString, 'arial', 24, COLOR_WHITE);
end;
...
begin
  ...
  Log1 ('VNC Server Test 10.');
  Log1 ('2018 pjde.');
   if SysLogger = nil then
    Log1 ('SysLogger inactive.')
  else
    Log1 ('SysLogger active.');
...
{$endif}
  ...
  aVNC.Canvas.Fill (SetRect (300, 150, 350, 200), COLOR_BLUE);
